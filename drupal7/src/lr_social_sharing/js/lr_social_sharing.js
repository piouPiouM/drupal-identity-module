var selected_sharing_theme = jQuery('[name="lr_social_sharing_selected_share_interface"]');
...
window.onload = function () {
    jQuery("#edit-lr-social-sharing-horizontal-images-0,#edit-lr-social-sharing-horizontal-images-1, #edit-lr-social-sharing-horizontal-images-10").click(function () {
        sharing_horizontal_show();
    });
    jQuery("#edit-lr-social-sharing-horizontal-images-2,#edit-lr-social-sharing-horizontal-images-3").click(function () {
        sharing_horizontal_show();
        sharing_simplehorizontal_show();
    });
    jQuery("#edit-lr-social-sharing-horizontal-images-8,#edit-lr-social-sharing-horizontal-images-9").click(function () {
        counter_horizontal_show();
    });
    jQuery("#edit-lr-social-sharing-vertical-images-4,#edit-lr-social-sharing-vertical-images-5").click(function () {
        sharing_vertical_show();
    });
    jQuery("#edit-lr-social-sharing-vertical-images-6,#edit-lr-social-sharing-vertical-images-7").click(function () {
        counter_vertical_show();
    });

    sharingproviderlist();
    counterproviderlist();
    jQuery("#lr_social_sharing_rearrange_providers, #socialshare_vertical_rearrange_providers").sortable({
        revert: true
    });
    if (selected_sharing_theme) {
        loginRadiusToggleShareTheme(selected_sharing_theme.val());
    }
    jQuery(".lr_social_sharing_rearrange_providers, .socialshare_vertical_rearrange_providers").find("li").unwrap();
    jQuery("#lr_social_sharing_veritical").click(function () {
        loginRadiusToggleShareTheme("vertical");
    });
    jQuery("#lr_social_sharing_horizontal").click(function () {
...
function loginRadiusToggleShareTheme(theme) {
    ...
    var lr_social_sharing_horizontal = jQuery("#lr_social_sharing_horizontal");
    var lr_social_sharing_veritical = jQuery("#lr_social_sharing_veritical");
    ...
    if (theme == "vertical") {
        ...
        lr_social_sharing_horizontal.removeClass("active");
        lr_social_sharing_veritical.addClass("active");
        vertical_position.removeClass("element-invisible");
    } else {
        lr_social_sharing_horizontal.addClass("active");
        lr_social_sharing_veritical.removeClass("active");
        vertical_position.addClass("element-invisible");
    }
    jQuery("#lr_social_sharing_horizontal_images").css("display", horizontalDisplay);
    jQuery("#lr_social_sharing_vertical_images").css("display", verticalDisplay);
}
/*
...
function sharingproviderlist() {
    ...
    if (div && div_vertical) {
        ...
        jQuery('input[name^="lr_social_sharing_rearrange_providers_list"]').each(function () {
...
function loginRadiusRearrangeProviderList(elem) {
    var ul = jQuery('#lr_social_sharing_rearrange_providers');
    var input = jQuery('#lr_social_sharing_chnage_name');
    if (elem.checked) {
        var provider = jQuery("<li id='edit-lrshare-iconsprite32" + elem.value + "' title='" + elem.value + "' class='lrshare_iconsprite32 lrshare_" + elem.value + "'><input type='hidden' value='" + elem.value + "' name='lr_social_sharing_rearrange_providers_list[]' id='input-lrshare-" + elem.value + "'></li>");
        ul.append(provider);
    } else {
...
function loginRadiusSharingLimit(elem) {
    var checkCount = jQuery('input[name^="lr_social_sharing_rearrange_providers_list"]').length;
    if (elem.checked) {
...
function toggle_sharing_counter(is_social_share, is_social_counter) {
    ...
    jQuery("#socialshare_providers_list, #rearrange_sharing_text, #lr_social_sharing_rearrange_providers")[simple_counter]("element-invisible");
...
function toggle_horizontal_widget(is_horizontal) {
    ...
    jQuery("#lr_social_sharing_show_horizotal_widget, .form-item.form-type-textarea.form-item-socialshare-show-exceptpages, .form-item.form-type-radios.form-item-socialshare-show-pages, #horizontal_sharing_show, .form-item.form-type-radios.form-item-lr-social-sharing-enable-horizontal-share, .form-item.form-type-select.form-item-socialshare-top-weight, .form-item.form-type-select.form-item-socialshare-bottom-weight, .form-item.form-type-radios.form-item-socialshare-horizontal-location, .form-item.form-type-textfield.form-item-socialshare-label-string")[horizontal_sharing]("element-invisible");
    jQuery("#lr_social_sharing_show_veritcal_widget, .form-item.form-type-radios.form-item-socialshare-vertical-show-pages, .form-item.form-type-textarea.form-item-socialshare-vertical-show-exceptpages, .form-item.form-type-radios.form-item-lr-social-sharing-enable-vertical-share, .form-item.form-type-radios.form-item-socialshare-vertical-location")[vertical_sharing]("element-invisible");
}
/*
