...
class SocialLoginUserManager {
  ...
  public function mapAccount($user_id, $provider_id, $provider) {   
     $raas_uid = $this->connection->query("SELECT am.lr_raas_uid FROM {users} am WHERE  am.uid = :uid", array(
    ...
    try {
      ...
      $result = $accountObj->accountLink($raas_uid, $provider_id, $provider);   
       ...
       if (isset($e->getErrorResponse()->description) && $e->getErrorResponse()->description) {
         $result = $e->getErrorResponse()->description;
        }
      }
  ...
  public function handleAccountLinking($userprofile) {
    if ($userprofile->ID != '' && !\Drupal::currentUser()->isAnonymous()) {
      ...
      if (count(\Drupal::moduleHandler()->getImplementations('add_user_identities_submit')) > 0) {
        // Call all modules that implement the hook, and let them make changes to $variables.
        \Drupal::moduleHandler()->invokeAll('add_user_identities_submit', $userprofile, $account);
      }
      //Advanced module LR Code Hook End
      ...
      if (empty($provider_id_exist) && !$provider_id_exist) {
        if (empty($provider_exist) && !$provider_exist) {
         $result = $this->mapAccount($user->id(), $userprofile->ID, $userprofile->Provider);
          if (isset($result->isPosted) && $result->isPosted) {
