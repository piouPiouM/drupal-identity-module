...
class SocialLoginUserManager {
  ...
  public function mapAccount($user_id, $provider_id, $provider) {
  ...
  public function getMappedAccounts($uid) {
    return $this->connection->query("SELECT am.uid, provider, provider_id FROM {users} am INNER JOIN {loginradius_mapusers} sm ON am.uid = sm.user_id WHERE uid = :uid", array(
      ':uid' => $uid
    ));
  }
  ...
  public function checkExistingUser($userprofile) {
    ...
    if (!$drupal_user) {
      ...
      if (!empty($userprofile->Email_value)) {
        ...
        if ($drupal_user) {
          $this->mapAccount($drupal_user->id(), $userprofile->ID, $userprofile->Provider);
        }
      }
  ...
  public function handleAccountLinking($userprofile) {
    if ($userprofile->ID != '' && !\Drupal::currentUser()->isAnonymous()) {
      ...
      $provider_id_exist = $this->connection->query("SELECT am.uid FROM {users} am INNER JOIN {loginradius_mapusers} sm ON am.uid = sm.user_id WHERE  sm.provider_id = :provider_id", array(
        ...
        ->fetchField();
      $provider_exist = $this->connection->query("SELECT am.uid FROM {users} am INNER JOIN {loginradius_mapusers} sm ON am.uid = sm.user_id WHERE  sm.provider = :provider", array(
        ':provider' => $userprofile->Provider,
      ))
        ->fetchField();
      if (empty($provider_id_exist) && !$provider_id_exist) {
        if (empty($provider_exist) && !$provider_exist) {
          $this->mapAccount($user->id(), $userprofile->ID, $userprofile->Provider);
